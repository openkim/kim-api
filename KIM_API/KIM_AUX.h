            ...
            *ier=KIM_STATUS_FAIL;
            ...
            if (halfNeighbors){
                ...
                if (virialPerAtom_flag==1 && pkim->virialPerAtom_need2add ){
                    ...
                    virialPerAtom[(*i)*6 + 0] += vir[0]*0.5;
                    virialPerAtom[(*i)*6 + 1] += vir[1]*0.5;
                    virialPerAtom[(*i)*6 + 2] += vir[2]*0.5;
                    virialPerAtom[(*i)*6 + 3] += vir[3]*0.5;
                    virialPerAtom[(*i)*6 + 4] += vir[4]*0.5;
                    virialPerAtom[(*i)*6 + 5] += vir[5]*0.5;

                    virialPerAtom[(*j)*6 + 0] += vir[0]*0.5;
                    virialPerAtom[(*j)*6 + 1] += vir[1]*0.5;
                    virialPerAtom[(*j)*6 + 2] += vir[2]*0.5;
                    virialPerAtom[(*j)*6 + 3] += vir[3]*0.5;
                    virialPerAtom[(*j)*6 + 4] += vir[4]*0.5;
                    virialPerAtom[(*j)*6 + 5] += vir[5]*0.5;
                }
            }else{
                v=v*0.5;
                vir[0] = v * dx[0] * dx[0];
