...
if test x"`basename "$0" 2> /dev/null`" \
        = x"${_full_package_name}-deactivate"; then
  (>&2 printf "Error:\t${_full_package_name}-deactivate must be sourced.\n"
   printf "\tRun 'source ${_full_package_name}-deactivate' not "
   printf "'${_full_package_name}-deactivate'.\n")
  exit 1
fi
...
for _func in `grep "_${_full_package_name}-.*[[:space:]]*()" \
  "${_sysconfdir}/bash_completion.d/${_full_package_name}-completion.bash"`; do
