...
get_local_build_item_name () {
  ...
  local item_type="`${make_command} kim-item-type`"
  case "${item_type}" in
    Model|ParameterizedModel|SimulatorModel)
      item_name="`${make_command} model-name`"
      ;;
    ModelDriver)
      item_name="`${make_command} model-driver-name`"
      ;;
    *)
      item_name=""
...
check_makefile_for_v2_build_config () {
  ...
  local item_build_config="`${make_command} print-KIM_API_BUILD_CONFIG 2> /dev/null`"
