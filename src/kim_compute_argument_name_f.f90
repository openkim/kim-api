...
module kim_compute_argument_name_f_module
  ...
  interface
    type(c_ptr) function compute_argument_name_string(compute_argument_name) &
      bind(c, name="KIM_Compute_ArgumentNameString")
    ...
    subroutine get_number_of_arguments(number_of_arguments) &
      bind(c, name="KIM_COMPUTE_ARGUMENT_NAME_get_number_of_arguments")
      use, intrinsic :: iso_c_binding
      integer(c_int), intent(out) :: number_of_arguments
    end subroutine get_number_of_arguments

    integer(c_int) function get_argument(index, argument_name) &
      bind(c, name="KIM_COMPUTE_ARGUMENT_NAME_get_argument")
      use, intrinsic :: iso_c_binding
      use kim_compute_argument_name_module, only : &
        kim_compute_argument_name_type
      implicit none
    ...
    integer(c_int) function get_argument_data_type(argument_name, data_type) &
      bind(c, name="KIM_COMPUTE_ARGUMENT_NAME_get_argument_data_type")
      use, intrinsic :: iso_c_binding
      use kim_compute_argument_name_module, only : &
        kim_compute_argument_name_type
...
subroutine kim_compute_argument_name_string(argument_name, name_string)
  ...
  use kim_compute_argument_name_f_module, only : compute_argument_name_string
  implicit none
  type(kim_compute_argument_name_type), intent(in), value :: argument_name
...
subroutine kim_compute_argument_name_get_number_of_arguments( &
  number_of_arguments)
  use, intrinsic :: iso_c_binding
  use kim_compute_argument_name_f_module, only : get_number_of_arguments
  implicit none
  integer(c_int), intent(out) :: number_of_arguments
...
subroutine kim_compute_argument_name_get_argument(index, argument_name, ierr)
  use, intrinsic :: iso_c_binding
  use kim_compute_argument_name_module, only : kim_compute_argument_name_type
  use kim_compute_argument_name_f_module, only : get_argument
  implicit none
  integer(c_int), intent(in), value :: index
  type(kim_compute_argument_name_type), intent(out) :: argument_name
  integer(c_int), intent(out) :: ierr

  ierr = get_argument(index-1, argument_name)
...
subroutine kim_compute_argument_name_get_argument_data_type(argument_name, &
  ...
  use kim_compute_argument_name_module, only : kim_compute_argument_name_type
  use kim_data_type_module, only : kim_data_type_type
  use kim_compute_argument_name_f_module, only : get_argument_data_type
  implicit none
  type(kim_compute_argument_name_type), intent(in), value :: argument_name
  type(kim_data_type_type), intent(out) :: data_type
  integer(c_int), intent(out) :: ierr
