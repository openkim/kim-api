...
program TEST_NAME_STR
  ...
  call create_FCC_configuration(Spacings(1), 2*CellsPerCutoff, .true., coords, MiddleAtomId)

  ! compute special neighbor list (Only the middle atom will have neighbors)
  allocate(neighborList(N+1, N))
  !
  call NEIGH_PURE_periodic_neighborlist(halfflag, N, coords, (cutoff+0.75), MiddleAtomId, neighborList)
  ...
  call create_FCC_configuration(Spacings(3), 2*CellsPerCutoff, .true., coords, MiddleAtomId)
  ...
  call create_FCC_configuration(Spacings(2), 2*CellsPerCutoff, .true., coords, MiddleAtomId)
  ! compute new neighbor lists (could be done more intelligently, I'm sure)
  call NEIGH_PURE_periodic_neighborlist(halfflag, N, coords, (cutoff+0.75), MiddleAtomId, neighborList)
  ! Call model compute
  call kim_api_model_compute_f(pkim, ier)
  ...
  do while (abs(Spacings(3) - Spacings(1)) .gt. TOL)
     ...
     call create_FCC_configuration(Spacings(4), 2*CellsPerCutoff, .true., coords, MiddleAtomId)
     ! compute new neighbor lists (could be done more intelligently, I'm sure)
     call NEIGH_PURE_periodic_neighborlist(halfflag, N, coords, (cutoff+0.75), MiddleAtomId, neighborList)
     ! Call model compute
     call kim_api_model_compute_f(pkim, ier)
