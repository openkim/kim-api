...
!**  the minimum energy of one cell in a periodic B2 crystal of SPECIES1_NAME_STR and SPECIES2_NAME_STR as a 
...
program TEST_NAME_STR
  ...
  N = 2; N4 = N ! We'll use just two atoms (one SPECIES1_NAME_STR and one SPECIES2_NAME_STR) for this calculation!
  ...
  atomTypes(1)    = kim_api_get_atypecode_f(pkim, "SPECIES1_NAME_STR", ier)
  if (ier.le.0) then
     call report_error(__LINE__, "kim_api_get_atypecode_f", ier)
     stop
  endif
  atomTypes(2)    = kim_api_get_atypecode_f(pkim, "SPECIES2_NAME_STR", ier)
  if (ier.le.0) then
     call report_error(__LINE__, "kim_api_get_atypecode_f", ier)
     stop
  endif
  ...
  call NEIGH_RVEC_F_periodic_B2_neighborlist(CellsPerCutoff, (cutoff+0.75),Spacings(1), NNeighbors, neighborList, RijList)
