...
module model_demo_P_LJ_CLUSTER_F
	use  KIMservice
	implicit none
			...
			do j=1, lj_obj%numberatoms
				if (i.ne.j) then
					...
					if (r2.le.cut2) then
						...
						if (e_flag.eq.1) ea(i)=ea(i) + (vij-energycutof)/2.0
...
end module model_demo_P_LJ_CLUSTER_F

!  Model Initiation routine (it calls actual initialization routine in the module model_demo_P_LJ_CLUSTER_F)
subroutine model_demo_p_lj_cluster_f_init(pkim)
	use model_demo_P_LJ_CLUSTER_F
	implicit none
	integer(kind=kim_intptr) :: kim; pointer(pkim,kim)
	call lj_init(pkim)
end subroutine model_demo_p_lj_cluster_f_init
