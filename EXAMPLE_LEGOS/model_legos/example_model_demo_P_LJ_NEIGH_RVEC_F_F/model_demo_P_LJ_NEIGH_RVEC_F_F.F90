...
module model_demo_P_LJ_NEIGH_RVEC_F_F
	use  KIMservice
	implicit none
		...
		retcode=1
			...
			do jj=1, numnei
				...
				if (r2.le.cut2) then
					...
					sumv = sumv + (vij-energycutof)/2.0d0
					if (e_flag.eq.1) ea(i)=ea(i)+(vij-energycutof)/2.0d0
...
end module model_demo_P_LJ_NEIGH_RVEC_F_F

!  Model Initiation routine (it calls actual initialization routine in the module demo_P_LJ_NEIGH_RVEC_F_F)
subroutine model_demo_p_lj_neigh_rvec_f_f_init(pkim)
	use model_demo_P_LJ_NEIGH_RVEC_F_F
	implicit none
	integer(kind=kim_intptr) :: kim; pointer(pkim,kim)
	call lj_init(pkim)
end subroutine model_demo_p_lj_neigh_rvec_f_f_init
