...
do i = 1,N
   if (comp_force.eq.1.or.comp_virial.eq.1) then
      call calc_U_dU(rho(i),Utmp,dU,ielast)       ! compute embedding energy
                                                  !   and its derivative
      derU(i) = dU                                ! store dU for later use
   else
      call calc_U(rho(i),Utmp,ielast)             ! compute just embedding energy
   endif
...
end module model_Al_PF_ErcolessiAdams
...
!
!-------------------------------------------------------------------------------
subroutine model_Al_PF_ErcolessiAdams_init(pkim)
use model_Al_PF_ErcolessiAdams
use KIMservice
implicit none
