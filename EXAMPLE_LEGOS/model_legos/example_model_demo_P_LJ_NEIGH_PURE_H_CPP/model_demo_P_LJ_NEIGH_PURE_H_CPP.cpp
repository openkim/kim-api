...
void model_demo_p_lj_neigh_pure_h_cpp_init_(void * km);
}
...
// 
namespace {
void model_demo_p_lj_neigh_pure_h_cpp_calculate(void *km,int*);

void ljpotr (double rr,double *v,double *dvmr);

void model_demo_p_lj_neigh_pure_h_cpp_destroy_(void * km,int * kimerror){
	cout<<endl<<"model_demo_p_lj_neigh_pure_h_cpp_destroy_ (void*)  called!"<<endl;
}
...
void model_demo_p_lj_neigh_pure_h_cpp_init_(void * km){
	...
	if (!pkim->set_data("compute",1,(void*)&model_demo_p_lj_neigh_pure_h_cpp_calculate)) {
		cout<< ":compute: is not in kim "<<endl;
		exit(335);
	}
	
	if (!pkim->set_data("destroy",1,(void*)&model_demo_p_lj_neigh_pure_h_cpp_destroy_)) {
...
void model_demo_p_lj_neigh_pure_h_cpp_calculate(void *km,int * kimerror){
	...
	for(i=0;i < (int) *numberOfAtoms;i++){
		...
		if(retcode!=1) {
			cout<<"model_demo_p_lj_neigh_pure_h_cpp_calculate: unsuccesful  get_half_neigh, retcode = "<<retcode<<endl;
			*kimerror = retcode;
			return;
		}
