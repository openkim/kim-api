...
ifdef KIM_DYNAMIC
   ...
   all: models_check $(KIM_CONFIG_FILES) kim-config-links kim-api-all $(patsubst %,%-all,$(MODEL_DRIVERS_LIST) $(MODELS_LIST)) \
...
# cleaning targets
clean: $(patsubst %,%-clean,$(MODELS_LIST) $(MODEL_DRIVERS_LIST) $(TESTS_LIST)) kim-api-clean clean-config
clean-examples:
...
$(KIM_CONFIG_FILES): $(KIM_DIR)/Makefile.KIMConfig $(KIM_DIR)/Makefile.LoadCompilerSettings $(KIM_API_DIR)/Makefile.GenericSettings
