...
install-set-default-to-v%:
	...
	$(QUELL)fl="$(DESTDIR)$(bindir)/$(package_name)-external-model"        && if test -L "$$fl"; then rm -f "$$fl"; fi && ln -fs "$(package_name)-v$*-external-model" "$$fl"
...
uninstall-set-default:
	...
	$(QUELL)fl="$(DESTDIR)$(bindir)/$(package_name)-external-model"        && if test -L "$$fl"; then rm -f "$$fl"; fi
