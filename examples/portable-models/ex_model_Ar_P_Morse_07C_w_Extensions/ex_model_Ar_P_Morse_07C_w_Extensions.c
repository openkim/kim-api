...
static int model_extension(KIM_ModelExtension * const modelExtension,
                           void * const extensionStructure)
{
  char const * extensionID;
  KIM_SupportedExtensions * supportedExtensions;

  KIM_ModelExtension_GetExtensionID(modelExtension, &extensionID);

  if (strcmp(extensionID, KIM_SUPPORTED_EXTENSIONS_ID) == 0)
  {
    supportedExtensions = (KIM_SupportedExtensions *) extensionStructure;

    supportedExtensions->numberOfSupportedExtensions = 2;
    strcpy(supportedExtensions->supportedExtensionID[0],
           KIM_SUPPORTED_EXTENSIONS_ID);
    supportedExtensions->supportedExtensionRequired[0] = 0;

    strcpy(supportedExtensions->supportedExtensionID[1], "Fake_Extension");
    supportedExtensions->supportedExtensionRequired[1] = 0;

    return FALSE;
  }
  else
  {
    LOG_ERROR("Unknown ExtensionID.");
    return TRUE;
  }
}
                 ...
                 KIM_TimeUnit const requestedTimeUnit)
{
  ...
  KIM_ModelExtensionFunction * extension = model_extension;
