...
static int compute(void* km)
{
   ...
   is_half = (KIM_API_is_half_neighbors(pkim, &ier));
   if (KIM_STATUS_OK > ier)
   {
      KIM_API_report_error(__LINE__, __FILE__, "KIM_API_is_half_neighbors", ier);
      return ier;
   }
   ...
   KIM_API_getm_data(pkim, &ier, 18*3,
                     ...
                     "numberContributingParticles", &numContrib,       (is_half==1),
   ...
   for (i = 0; i < *nAtoms; ++i)
   {
      ...
      {
         KIM_API_report_error(__LINE__, __FILE__, "Wrong atomSpecie", i);
         return ier;
      }
   }
      ...
      for (jj = 0; jj < numOfAtomNeigh; ++ jj)
      {
         ...
         if (Rsqij < *cutsq)
         {
            ...
            if (comp_particleEnergy)
            {
               ...
               if ((1 == is_half) && (j < numContrib)) particleEnergy[j] += 0.5*phi;
            ...
            else if (comp_energy)
            {
               if ((1 == is_half) && (j < numContrib))
               {
                  /* Half mode -- add phi to total energy */
                  *energy += phi;
               }
               else
               {
                  /* Full mode -- add half phi to total energy */
                  *energy += 0.5*phi;
               }
            }
